...
To play SRG SSR content, create a `PlayerItem` using ``PillarboxPlayer/PlayerItem/urn(_:server:trackerAdapters:configuration:commandersActSource:)``, and pass the content URN as a parameter. Then load this `PlayerItem` into a `Player` instance.
