...
final class URLAssetDownloadStore<Provider> where Provider: AssetProvider, Provider.CustomData: Codable {
...
private extension URLAssetDownloadStore {
    ...
    final class URLEntry {
        ...
        init(id: String, record: DownloadRecord<URLAssetLoader<Provider>.Input, Provider.CustomData>) {
            self.id = id
        ...
        func toRecord() -> DownloadRecord<URLAssetLoader<Provider>.Input, Provider.CustomData> {
            let assetMetadata = metadata.assetMetadata()
            return .init(
                // TODO: Configuration
                input: .init(url: url, configuration: .default, metadata: assetMetadata),
        ...
        func update(with record: DownloadRecord<URLAssetLoader<Provider>.Input, Provider.CustomData>) {
            self.url = record.input.url
            self.configuration = record.configuration
            self.metadata = .init(assetMetadata: record.input.metadata)
            self.bookmarkData = record.bookmarkData
...
extension URLAssetDownloadStore: AssetDownloadStore {
    ...
    static func id(from input: URLAssetLoader<Provider>.Input) -> String {
    ...
    func downloadRecords() -> [DownloadRecord<URLAssetLoader<Provider>.Input, Provider.CustomData>] {
        guard let entries = try? context.fetch(FetchDescriptor<URLEntry>()) else { return [] }
        return entries.map { $0.toRecord() }
    }

    func addDownloadRecord(_ record: DownloadRecord<URLAssetLoader<Provider>.Input, Provider.CustomData>, forId id: String) {
    ...
    func downloadRecord(forId id: String) -> DownloadRecord<URLAssetLoader<Provider>.Input, Provider.CustomData>? {
        entry(forId: id)?.toRecord()
    }

    func updateDownloadRecord(_ record: DownloadRecord<URLAssetLoader<Provider>.Input, Provider.CustomData>, forId id: String) {
