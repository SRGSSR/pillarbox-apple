...
final class PlayerItemTests: TestCase {
    ...
    func testSimpleItem() {
        ...
        let playerItem = item.content.playerItem(reload: false, configuration: .default, limits: .none)
    ...
    func testSimpleItemWithConfiguration() {
        ...
        PlayerItem.load(for: item.id)
        expect(item.content.resource).toEventually(equal(.simple(url: Stream.onDemand.url)))
        expect(item.content.playerItem(reload: false, configuration: .default, limits: .none).preferredForwardBufferDuration).to(equal(4))
    }
    ...
    func testSimpleItemWithLimits() {
        ...
        PlayerItem.load(for: item.id)
        expect(item.content.resource).toEventually(equal(.simple(url: Stream.onDemand.url)))
        let playerItem = item.content.playerItem(reload: false, configuration: .default, limits: Self.limits)
